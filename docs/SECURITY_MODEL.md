...
- **A Púca Notes sign-in on the phone app stays signed in for up to a year unless you say otherwise.**
  ...
  until the year is up, unless the session is ended. A renewal never extends a token past
  the year: its expiry is clamped to it. (The one exception is transitional: a token that a
  server from before that clamp renewed in the session's last month keeps the expiry it was
  given, at most 30 days past the year.) From another device, ending the session takes *Sign
...
- For a personal note (Púca Notes, migration 065): whether it has text,
  ...
  transcribed. A web address inside a note's text or an item is found and
  rendered as a link entirely on the device, with no request of any kind, so
  neither the server nor the site learns that the note holds a link or that it
  is being read (`docs/NOTES.md`, *Links in a note*).
...
- **Púca Notes sync metadata.** A note's colour, labels and archive flag, plus
  ...
  earlier colours and labels). That memory holds the last synced document in
  clear in localStorage, so a sync still in flight at sign-out must not write
  it back after the scrub: every sync step re-checks the account and a
  sign-out counter after each await and does nothing if either moved, and the
  app's record store refuses any account but the signed-in one. The same check
  keeps one account's colours and labels out of the next account's copy when
  both sign in on the same page without a reload. A refused older copy is shown, not applied, and
...
- **`due_at`, in plaintext.** For a plain dated item that is its due time, as it
  ...
  but what they produce is an ordinary `due_at`. The switch **Keep the time
  private from the server** (off by default) keeps `due_at` NULL, and it sits
  on the same dialog as the preset row. It works per item and per note alike
  — a note has its own sealed schedule for the time to live in — and what it
  hides then shows in the calendar but cannot notify from the server's
  reminder feed.
