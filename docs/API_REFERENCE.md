...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| GET | `/task-features` | ✅ | `{"version":1,"features":["schedule","snooze","updated_at","expect_due_at","recurrence_aware","reopen_subtree","reminder_feed_v2","op_key"]}`. Static. A server older than 066 answers 404, which clients read as "none of these" — the probe works for a note with no tasks, where a `[]` list response carries no keys to detect. |
