...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| GET | `/task-features` | ✅ | `{"version":1,"features":["schedule","snooze","updated_at","expect_due_at","recurrence_aware","expect_schedules_as_of","reopen_subtree","reminder_feed_v2","op_key"]}`. Static. A server older than 066 answers 404, which clients read as "none of these" — the probe works for a note with no tasks, where a `[]` list response carries no keys to detect. |
| PATCH | `/tasks/:task_id` | ✅ | Adds `schedule` (creator or MANAGE_TASKS, like the description; sealed cap 16 KiB), `snooze` (COMPLETE_TASKS or MANAGE_TASKS, like ticking; cap 1 KiB), `expect_due_at` (compare-and-swap on `due_at`: `""` = expect NULL; a mismatch is 409 and nothing is written), `recurrence_aware` (default false: completing an item whose subtree carries any schedule is **409** without it, so an older client cannot silently end a repeating series), `expect_schedules_as_of` (RFC3339: the newest `updated_at` the client saw on the item — and, for a completion, on its subtree — echoed as the server rendered it; an open row with a schedule changed after it is **409** and nothing is written; absent = no check; not RFC3339 = 400) and `reopen_subtree` (reopens everything under the item). The envelope downgrade guard covers `schedule` and `snooze`. |
