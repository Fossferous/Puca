...
| GET | `/files/:id` | ✅ | Get file. A file uploaded with a capability is served only to a caller presenting it in request header `X-Puca-File-Cap`; a missing or wrong one is a 404 (no existence oracle). This enforcement is **on by default**; `FILES_ENFORCE_CAP=0` on the server drops back to checking a capability only when one is presented, which lets a client older than 0.8.134 keep fetching but also lets any signed-in account that learns a file id fetch it. A file **without** a capability is not public either: it is served to its uploader; an avatar or join/leave sound to accounts in its owner's identity context (friends, a shared server, or a conversation the owner started); a server icon or emoji to that server's members (or to everyone once the server is public, as `/discover` already shows it); a clip part to anyone who can view the channel the clip was posted in. Uploads that predate migration 054 keep working for every signed-in account — nothing can scope them. Anything else (a post-capability upload that asked for no capability and is referenced by nothing) is its uploader's alone, which is the 404 an attachment some client forgot to protect now gets from a stranger who learned its id (found 2026-09-07). `FILES_ENFORCE_CAP=0` lifts this scoping too. |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| GET | `/task-lists/features` | ✅ | What this server supports for lists: `{ body, attachments, trash, trash_retention_days, max_body_len, server_now_ms, content_rev, idempotent_creates }` (`trash_retention_days` 0 = kept until emptied; `server_now_ms` is the server's clock, which a client purging its own expired trash measures against instead of the device clock; `content_rev` = this server honours `expect_rev`, `idempotent_creates` = it honours `op_key`). An older server answers with an error (405), or omits the newer keys, which clients read as "none of it" — the detection does not depend on the account having lists. |
| GET | `/task-lists` | ✅ | Your live lists with progress counts, `body`, `attachments`, `trashed_at` (null on every live list), `is_self` (true only for the Notes-to-self list, which cannot be trashed) and `content_rev` (always present from a 069 server: how many times this note's own `title`/`body`/`attachments` were written — an item write never moves it). Trashed lists are not listed. `?trashed=true` lists only the trash, most recently trashed first. |
| POST | `/task-lists` | ✅ | Create a list: `title` (required), optional `body` and `attachments`. Optional `op_key`: a RANDOM client id for this one create, 16–64 url-safe characters (a bad shape is a 400), repeated on every retry. A replay of a key already used is answered **200 with the note that create already made** — same id, same `created_at` — and nothing new is written; 409 if that note has since been deleted, or if that key was spent on an item rather than a note (a key belongs to one create, and the two id spaces are independent). Absent = today's unguarded create, so older clients are unaffected. Keys are per account and are swept after `NOTES_OP_KEY_RETENTION_HOURS` (default 24; 0 keeps them). |
| PATCH | `/task-lists/:id` | ✅ | Change `title`, `body` and/or `attachments`; each is optional (absent = unchanged, `""` clears `body`/`attachments`), an empty request is a 400. Optional `expect_rev`: the `content_rev` this edit is based on — a mismatch is **409 with a JSON body** `{"conflict":"stale","content_rev":…,"title":…,"body":…,"attachments":…}` (the sealed current copy, exactly what a GET would have returned) and **nothing is written**; absent = no check. A success answers `{ "content_rev": <new> }`, so a run of saves needs no refetch between them. Still 409 (plain text) while the list is in the trash, and 409 for an edit that would downgrade a newer envelope (`reads_up_to`, as for tasks) — a client must treat an unrecognised 409 as it always did. `body` over 64 KiB of envelope is a 413. |
...
carries `schedule`, `snooze` and `updated_at` keys (null when unset), and from
migration 071 `schedule_changed_at`: when the item was created or its
schedule, tick or parent last changed (rendered like `updated_at`; a text,
attachment, due-time or snooze edit does not move it). What the server can
see of them is in `docs/SECURITY_MODEL.md` §2.

| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| GET | `/task-features` | ✅ | `{"version":1,"features":["schedule","snooze","updated_at","schedule_changed_at","expect_due_at","recurrence_aware","expect_schedules_as_of","reopen_subtree","reminder_feed_v2","op_key"]}`. Static. A server older than 066 answers 404, which clients read as "none of these" — the probe works for a note with no tasks, where a `[]` list response carries no keys to detect. |
| PATCH | `/tasks/:task_id` | ✅ | Adds `schedule` (creator or MANAGE_TASKS, like the description; sealed cap 16 KiB), `snooze` (COMPLETE_TASKS or MANAGE_TASKS, like ticking; cap 1 KiB), `expect_due_at` (compare-and-swap on `due_at`: `""` = expect NULL; a mismatch is 409 and nothing is written), `recurrence_aware` (default false: completing an item whose subtree carries any schedule is **409** without it, so an older client cannot silently end a repeating series), `expect_schedules_as_of` (RFC3339: the newest `schedule_changed_at` the client saw on the item — and, for a completion, on its subtree; `updated_at` from a server older than 071, which is never behind it — echoed as the server rendered it. A completion is **409** when an open row with a schedule under it — or a row between that one and the item — changed after it; an advance (anything stamped that is not a completion) is **409** when the item itself changed after it, its repeat removed included, or when it has no schedule at all. The item's own row is judged again inside the write, so a change committed during the request is refused too, and nothing is written; a row deleted meanwhile is 404. Absent = no check; not RFC3339 = 400) and `reopen_subtree` (reopens everything under the item). The envelope downgrade guard covers `schedule` and `snooze`. |
| POST | `/task-lists/:list_id/tasks` | ✅ | Also accepts `schedule` (as does `POST /channels/:channel_id/tasks`), so an item and its timing land in one request. Both create routes also accept `op_key`, with the same contract as `POST /task-lists`: a replay is answered with the item that create already made (same id, same position), and no `ChecklistUpdate` is broadcast for it; a key spent on a note is a 409 here, and vice versa. A replay is recognised **before** the request is judged as a new one, so a checklist standing at its 2000-item limit — or one whose parent item has since been deleted — still answers the retry with the item rather than a 400 the client would read as "it was never saved". |
