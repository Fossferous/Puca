...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!("WebSocket path: /ws, authenticating with the Sec-WebSocket-Protocol header `bearer, YOUR_JWT` (?token= is REFUSED since 0.9.1 — it landed in access logs; every client since 0.9.0 sends the header) — reachable locally as ws://{addr}/ws, or wss://<public-host>/ws through the reverse proxy in production");
