...
function EntryMenu({
    ...
    // cannot drift apart. A note's OWN reminder has neither a completion nor
    // a snooze behind it (migration 068 adds no snooze column), so the menu
    // offers neither.
    const canSnooze = !!onSnooze && !entry.completed && !entry.source.isNote
        && maySnooze(t, entry.source.canComplete !== false, entry.source.canEdit);
