...
import { ENC_SIGN_IN, ENC_UNVERIFIED_SENDER, ENC_CANNOT_DECRYPT } from './decryptMarkers';
...
export async function encryptDMContent(content: string, recipientUserId: number): Promise<string> {
    ...
    const env = await encryptDM(identity, recipientPublicKey, content);
    if (!env) {
 ...
 */
export async function decryptDMContent(content: string, partnerUserId: number): Promise<string> {
    ...
    if (!env || (env.t !== 'dm' && env.t !== 'self')) return content;
    ...
    const plain = await decryptDM(identity, partnerKey, env);
...
export async function decryptDMMessages(messages: DMMessage[], partnerUserId: number): Promise<DMMessage[]> {
    return Promise.all(
        messages.map(async (msg) => {
            ...
            const text = await decryptDMContent(wire, partnerUserId);
