/**
 * "Take a note…" — Keep's composer. Collapsed it is one line; open it is a
 * title plus a growing list of items (Enter = next item). Closing with
 * content SAVES (Keep's behaviour): a personal list is created, then its
...
import { MAX_TITLE_LENGTH, cleanQuickItems } from '../model/keepModel';
...
export function QuickAdd({ onCreate, sheet = false, onDismiss, openSignal = 0 }: QuickAddProps) {
    ...
    // Click outside (desktop inline card) saves, like Keep.
    useEffect(() => {
    ...
    if (!open) {
        return (
            <div className="keep-quickadd" ref={rootRef}>
                <button
                    type="button"
                    className="keep-quickadd-collapsed"
    ...
    const body = (
        <div className={`keep-quickadd ${sheet ? 'sheet' : ''}`} ref={rootRef} role="dialog" aria-label="New note">
            <div className="keep-quickadd-open">
                <input
                    className="keep-quickadd-title"
                    placeholder="Title"
                    value={title}
                ...
                />
                {items.map((it, i) => (
                    <div className="keep-quickadd-item" key={i}>
                        <CheckboxIcon />
                        <input
                ...
                <div className="keep-quickadd-foot">
                    <button type="button" className="keep-iconbtn small" aria-label="Add item" title="Add item"
                        onClick={() => { setItems(prev => [...prev, '']); focusItem(items.length); }}>
                        <PlusIcon />
                    </button>
                    <button type="button" className="keep-iconbtn small" aria-label="Discard note" title="Discard" onClick={discard}>
                        <TrashIcon />
                    </button>
                    <button type="button" className="keep-textbtn" onClick={() => void close()} disabled={saving}>
                        {saving ? 'Saving…' : 'Done'}
                    </button>
                </div>
            </div>
