/**
 * The open note: Púca's TaskTree, with every callback wired, inside a Keep
 * frame (title, chips, add-item row ON TOP like Púca's Tasks editor — a
 * bottom-docked input sits under the Android keyboard — and the Keep tools
 ...
 * (keep.css), and never inside the drawer's transform.
 *
...
import { MAX_TITLE_LENGTH, type NoteCard } from '../model/keepModel';
...
export function NoteEditor({ card, actions, onClose, onMenu, onPickColor, onPickLabels, onArchive, pucaHref, escapeBlocked = false }: NoteEditorProps) {
    ...
    return createPortal(
        <div className="keep-editor-backdrop" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="keep-editor" data-color={card.color} role="dialog" aria-modal="true" aria-label={card.title || 'Untitled note'}>
                <div className="keep-editor-head">
                    <input
                    ...
                    <button
                        type="button"
                        className={`keep-iconbtn ${card.pinned ? 'active' : ''}`}
                    ...
                    <button type="button" className="keep-iconbtn" aria-label="Close note" title="Close (Esc)" onClick={onClose}>
                        <CloseIcon size={20} />
                    </button>
                </div>
                <div className="keep-editor-sub">
                    {card.serverName && <span className="keep-chip shared"><MembersIcon /> {card.serverName}</span>}
                    ...
                    {card.archived && <span className="keep-chip archived"><ArchiveIcon /> archived</span>}
                    {card.labels.map(l => <span key={l} className="keep-chip"><TagIcon /> {l}</span>)}
                    {card.createdAt && <span>Created {new Date(card.createdAt).toLocaleDateString()}</span>}
                    {tasksQuery.isFetching && <span className="keep-spinner" aria-label="Refreshing" />}
                </div>

                {titleUnreadable && (
                    <div className="keep-editor-locked"><LockIcon /> This note's title can't be read yet: {card.title}</div>
                )}

                <div className="keep-editor-body">
                    {canCreate && (
                        <form className="keep-editor-add" onSubmit={addItem}>
                            <input
                                ref={addRef}
                                value={newItem}
                    ...
                    {tasksQuery.isPending && tasks.length === 0 ? (
                        <div className="keep-loading"><span className="keep-spinner" /> Loading…</div>
                    ) : isChannel ? (
                        <TaskTree
                            tasks={tasks}
                ...
                <div className="keep-editor-foot">
                    <button type="button" className="keep-iconbtn" aria-label="Colour" title="Colour" onClick={e => onPickColor(card, e.currentTarget)}><PaletteIcon /></button>
                    <button type="button" className="keep-iconbtn" aria-label="Labels" title="Labels" onClick={e => onPickLabels(card, e.currentTarget)}><TagIcon /></button>
                    <button type="button" className="keep-iconbtn" aria-label={card.archived ? 'Unarchive' : 'Archive'} title={card.archived ? 'Unarchive' : 'Archive'} onClick={() => onArchive(card, !card.archived)}><ArchiveIcon /></button>
                    <button type="button" className="keep-iconbtn" aria-label="Refresh this note" title="Refresh" onClick={() => void actions.refreshNote(ref)}><RefreshIcon /></button>
                    {pucaHref && (
                        <a className="keep-iconbtn" href={pucaHref} target="_blank" rel="noopener" aria-label="Open in Púca" title="Open in Púca"><PopOutIcon /></a>
                    )}
                    <span className="spacer" />
                    <button type="button" className="keep-iconbtn" aria-label="More actions" title="More" onClick={e => onMenu(e, card, e.currentTarget)}><MoreVerticalIcon /></button>
                    <button type="button" className="keep-textbtn" onClick={onClose}>Close</button>
                </div>
            </div>
        </div>,
        document.body,
