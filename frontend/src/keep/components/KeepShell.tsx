/**
 * The signed-in Keep: top bar, rail, the grid or reminders view, the open
...
} from '../model/keepModel';
import { setKeepSort, setKeepView, type KeepSortMode } from '../model/keepPrefs';
import { useKeepPrefs, useNoteActions, useNoteCards } from '../model/notesQueries';
...
import { ShortcutsHelp } from './KeepDialog';
import { KeepRail } from './KeepRail';
import { KeepTopBar } from './KeepTopBar';
...
import { useKeepShortcuts } from './useKeepShortcuts';
...
function sortCards(cards: NoteCard[], sort: KeepSortMode): NoteCard[] {
...
interface KeepShellProps {
    onSignOut: () => void;
}

export function KeepShell({ onSignOut }: KeepShellProps) {
    ...
    const visible = useMemo(() => sortCards(filterNotes(cards, filter), keep.sort), [cards, filter, keep.sort]);
    ...
    const menuFor = (card: NoteCard, anchor: HTMLElement): ContextMenuItem[] => {
        ...
        const canMove = keep.sort === 'puca' && filter.kind !== 'search' && section.length > 1;
    ...
    const exportMd = () => downloadTextFile(`puca-keep-${fileStamp(Date.now())}.md`, notesToMarkdown(cards), 'text/markdown;charset=utf-8');
    const exportJson = () => downloadTextFile(`puca-keep-${fileStamp(Date.now())}.json`, notesToJson(cards, new Date().toISOString()), 'application/json');
    ...
    return (
        <div className="keep-app">
            ...
            <KeepTopBar
                ...
                view={keep.view}
                onToggleView={() => setKeepView(keep.view === 'grid' ? 'list' : 'grid')}
            ...
            <div className="keep-body">
                ...
                <main className="keep-main">
                    <div className="keep-main-inner">
                        {offline && (
                            <div className="keep-status offline" role="status">
                        ...
                        {error != null && !offline && (
                            <div className="keep-status error" role="alert">
                        ...
                        {remindersView ? (
                            ...
                            <>
                                ...
                                {filter.kind === 'label' && <h1 className="keep-section-title">Label: {filter.label}</h1>}
                                {filter.kind === 'archive' && <h1 className="keep-section-title">Archive</h1>}
                                {filter.kind === 'search' && <h1 className="keep-section-title">Results for “{filter.query}”</h1>}
                                <NoteGrid
                                    pinned={pinned}
                                    others={others}
                                    filter={filter}
            ...
            {!remindersView && (
                <button type="button" className="keep-fab" aria-label="New note" title="New note" onClick={() => setSheet(true)}>
                    <PlusIcon />
                </button>
            )}
