...
import { type NoteCard as NoteCardModel, previewRows, nearestDue } from '../model/keepModel';
...
function ImageThumb({ refItem, visible }: { refItem: TaskAttachmentRef; visible: boolean }) {
    ...
    if (!parsed || failed) return <span className="keep-thumb file" title={refItem.name}><WarningIcon /></span>;
    if (!url) return <span className="keep-thumb pending" title={refItem.name} />;
    return <img className="keep-thumb" src={url} alt={refItem.name} title={refItem.name} loading="lazy" />;
}
...
function NoteCardImpl({
    ...
    return (
        <article
            ...
            className={`keep-card ${card.pinned ? 'pinned' : ''}`}
        ...
        >
            <div className="keep-card-head">
                <h3 className={`keep-card-title ${untitled ? 'untitled' : ''}`}>
                ...
                <button
                    type="button"
                    className={`keep-iconbtn small keep-card-pin ${card.pinned ? 'pinned' : ''}`}
            ...
            {preview === null ? (
                <div className="keep-card-empty">Loading…</div>
            ) : preview.rows.length === 0 && preview.completedCount === 0 ? (
                <div className="keep-card-empty">Empty note</div>
            ) : (
                <ul className="keep-card-items">
                    {preview.rows.map(({ task, depth }) => (
                        <li key={task.id} className="keep-card-item" style={{ '--depth': depth } as React.CSSProperties}>
                            <input
                                type="checkbox"
                            ...
                            />
                            <span className="keep-card-item-text">{task.description}</span>
            ...
            {(thumbs.length > 0 || fileCount > 0) && (
                <div className="keep-card-thumbs">
                    {thumbs.map(r => <ImageThumb key={r.href} refItem={r} visible={onScreen} />)}
                    {fileCount > 0 && <span className="keep-chip">{fileCount} file{fileCount === 1 ? '' : 's'}</span>}
                </div>
            )}

            <div className="keep-card-foot">
                {card.serverName && (
                    <span className="keep-chip shared" title={`Shared checklist in ${card.serverName}`}><MembersIcon /> {card.serverName}</span>
                )}
                {due && (
                    <span className={`keep-chip ${isTaskOverdue(due, now) ? 'overdue' : ''}`} title={`Next due: ${new Date(due.due_at!).toLocaleString()}`}>
                        <ClockIcon /> {formatDueShort(due.due_at!, now)}
                    </span>
                )}
                {anyLocked && <span className="keep-chip" title="Some attachments can't be read yet (key unavailable)"><LockIcon /> locked</span>}
                {card.archived && <span className="keep-chip archived"><ArchiveIcon /> archived</span>}
                {card.total > 0 && <span className="keep-chip progress" title="Completed / total">{card.completed}/{card.total}</span>}
                {card.labels.map(l => (
                    <span
                        key={l}
                        className="keep-chip clickable"
                        role="button"
                        tabIndex={0}
            ...
            <div className="keep-card-tools" onClick={e => e.stopPropagation()}>
                {!compactTools && (
                    <>
