/**
 * Keep's single-key shortcuts. Every one is guarded by hotkeys.ts's
 ...
 * are installed only by registerHold/registerPress, which Keep never calls.
 *
...
export interface KeepShortcutHandlers {
...
export function useKeepShortcuts(h: KeepShortcutHandlers, enabled = true): void {
    const ref = useRef(h);
