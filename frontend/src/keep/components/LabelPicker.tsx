 ...
 * Labels are device-local (keepPrefs.ts) — the hint says so, once, where
 * the user is about to rely on them.
 */
import { useState } from 'react';
import { PlusIcon } from '../../components/Icons';
import { MAX_LABELS_PER_NOTE, MAX_LABEL_LENGTH, normalizeLabel } from '../model/keepModel';
...
export function LabelPicker({ all, value, onChange }: LabelPickerProps) {
    ...
    return (
        <div>
            <h4>Labels</h4>
            {options.length === 0 ? (
                <div className="keep-labels-hint">No labels yet — add one below.</div>
            ) : (
                <div className="keep-labels-list">
                    {options.map(l => (
                        <label key={l}>
                            <input
                                type="checkbox"
            ...
            <form className="keep-labels-new" onSubmit={e => { e.preventDefault(); add(); }}>
                <input
                    value={draft}
                    ...
                    disabled={full}
                />
                <button type="submit" className="keep-iconbtn small" aria-label="Add label" title="Add label" disabled={full || !normalizeLabel(draft)}>
                    <PlusIcon />
                </button>
            </form>
            <div className="keep-labels-hint">
