/**
 * Púca Keep — the data layer.
 *
 * Everything Keep shows comes from Púca's task API (api/tasks.ts) through
 ...
 * in Keep lands in exactly the state Púca would have produced — and rolls back
 ...
 * a delivered-once offer into nothing. Keep never opens the socket, so it can
...
import { getKeepPrefs, subscribeKeepPrefs, pruneKeepPrefs, setNoteArchived, setNoteColor, setNoteLabels } from './keepPrefs';

/** Keep's own client: it WANTS refetch-on-focus (that is its live sync),
...
export const keepKeys = {
    all: ['keep'] as const,
    lists: ['keep', 'lists'] as const,
    prefs: ['keep', 'prefs'] as const,
    servers: ['keep', 'servers'] as const,
    channels: (serverId: string) => ['keep', 'servers', serverId, 'channels'] as const,
    members: (serverId: string) => ['keep', 'servers', serverId, 'members'] as const,
    tasks: (ref: NoteRef) => ['keep', 'tasks', ref.kind, ref.id] as const,
};
...
export function useTaskListsQuery() {
    return useQuery({ queryKey: keepKeys.lists, queryFn: listTaskLists });
}
...
export function useServersQuery() {
    return useQuery({ queryKey: keepKeys.servers, queryFn: listServers });
}
...
export function useChannelSources(): { sources: NoteSource[]; complete: boolean } {
    ...
    const channelQueries = useQueries({
        queries: servers.map((s: Server) => ({
            queryKey: keepKeys.channels(s.id),
            queryFn: () => listChannels(s.id),
        })),
    });
    const memberQueries = useQueries({
        queries: servers.map((s: Server) => ({
            queryKey: keepKeys.members(s.id),
...
export function useNoteTasks(ref: NoteRef | null, opts: { live?: boolean } = {}) {
    return useQuery({
        queryKey: ref ? keepKeys.tasks(ref) : ['keep', 'tasks', 'none'],
...
export function useNoteCards(): {
    ...
    const keep = useKeepPrefs();
    const tasks = useAllNoteTasks(sources);
    const cards = useMemo(
        () => buildNoteCards(sources, tasks.byKey, prefs, keep),
        [sources, tasks.byKey, prefs, keep],
    );
    useEffect(() => {
...
export function useNoteActions(cards: NoteCard[], prefs: TaskTabPref[], prefsReady: boolean): NoteActions {
    ...
    const setTasks = useCallback((note: NoteRef, fn: (prev: Task[]) => Task[]) => {
        qc.setQueryData<Task[]>(keepKeys.tasks(note), prev => fn(prev ?? []));
    }, [qc]);
    ...
    const snapshot = useCallback(async (note: NoteRef): Promise<Task[]> => {
        await qc.cancelQueries({ queryKey: keepKeys.tasks(note) });
        return qc.getQueryData<Task[]>(keepKeys.tasks(note)) ?? [];
    }, [qc]);
    const restore = useCallback((note: NoteRef, tasks: Task[]) => { qc.setQueryData<Task[]>(keepKeys.tasks(note), tasks); }, [qc]);
    const syncListCounts = useCallback((note: NoteRef, tasks: Task[]) => {
        if (note.kind !== 'list') return;
        qc.setQueryData<TaskList[]>(keepKeys.lists, prev => prev?.map(l => l.id === note.id
    ...
    const createNote = useCallback(async (title: string, items: string[]): Promise<NoteRef | null> => {
        ...
        qc.setQueryData<Task[]>(keepKeys.tasks(ref), created);
        qc.setQueryData<TaskList[]>(keepKeys.lists, prev => [
            ...(prev ?? []),
    ...
    const renameNote = useCallback(async (note: NoteRef, title: string): Promise<boolean> => {
        ...
        const prev = qc.getQueryData<TaskList[]>(keepKeys.lists);
        qc.setQueryData<TaskList[]>(keepKeys.lists, p => p?.map(l => (l.id === note.id ? { ...l, title } : l)));
        try {
            await renameTaskList(note.id, title);
            return true;
        } catch (err) {
            explain('rename failed', err);
            qc.setQueryData(keepKeys.lists, prev);
            return false;
        }
    }, [qc]);
    ...
    const deleteNote = useCallback(async (note: NoteRef): Promise<boolean> => {
        ...
        qc.setQueryData<TaskList[]>(keepKeys.lists, p => p?.filter(l => l.id !== note.id));
        try {
            await deleteTaskList(note.id);
            qc.removeQueries({ queryKey: keepKeys.tasks(note) });
            return true;
        } catch (err) {
    ...
    const savePrefs = useCallback((next: TaskTabPref[]) => {
        ...
        qc.setQueryData<TaskTabPref[]>(keepKeys.prefs, next);
        putTaskTabPrefs(next).catch(err => {
            console.error('[keep] saving pins/order failed:', err);
            pushMessageToast({ title: 'Couldn’t save the pin or order — check your connection' });
            if (prefSeq.current === seq) qc.setQueryData<TaskTabPref[]>(keepKeys.prefs, before);
        });
    }, [qc]);
    ...
    const refreshAll = useCallback(async () => {
        await qc.invalidateQueries({ queryKey: keepKeys.all });
    }, [qc]);
    const refreshNote = useCallback(async (note: NoteRef) => {
        await qc.invalidateQueries({ queryKey: keepKeys.tasks(note) });
    }, [qc]);
