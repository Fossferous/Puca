/**
 * Púca Keep — the pure note model.
 *
 * Keep is a second front door onto Púca's task system: a "note" is a personal
 ...
 * Keep prefs) into what the grid, the reminders view and search render, and it
 * is unit-tested as such.
 *
 * Every ordering/favourite/toggle rule is DELEGATED to api/tasks.ts (the same
 * helpers TasksView and ChecklistBody use), so Keep cannot disagree with Púca
 ...
 * a translucent overlay defined in keep.css so it reads on all eight themes.
...
/** The device-local Keep state a card needs (see keepPrefs.ts). */
export interface KeepNoteState {
...
export function buildNoteCards(
    ...
    return ordered.map(s => {
        ...
        return {
            ...
            color: keep.colors[key] ?? 'default',
            labels: keep.labels[key] ?? [],
            archived: keep.archived[key] === true,
            total,
            completed,
        };
    });
...
/**
 * Apply a filter. `all` and `label` hide archived notes (Keep's behaviour);
 ...
 * rejects an empty one), while Keep lets you start typing items straight
