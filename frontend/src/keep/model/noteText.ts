/**
 * Púca Keep — notes as text: copy-as-text, and the export the account menu
...
import { type NoteCard } from './keepModel';
...
export function notesToJson(cards: NoteCard[], exportedAt: string): string {
    ...
    return JSON.stringify({ app: 'Púca Keep', exportedAt, notes }, null, 2) + '\n';
}
...
/** Hand the browser a file to save. Web only — Keep never runs in a shell. */
