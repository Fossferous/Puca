// Unit tests for the Púca Keep note model: the pure functions that turn
// lists + tasks + Púca's tab prefs + the device-local Keep prefs into cards,
...
import {
    ...
    type NoteSource, type NoteCard, type KeepNoteState,
} from '../keep/model/keepModel';
...
const NO_KEEP: KeepNoteState = { colors: {}, labels: {}, archived: {} };

function cards(
    sources: NoteSource[],
    tasks: Record<string, Task[]> = {},
    prefs: TaskTabPref[] = [],
    keep: KeepNoteState = NO_KEEP,
): NoteCard[] {
    return buildNoteCards(sources, new Map(Object.entries(tasks)), prefs, keep);
}
...
describe('buildNoteCards', () => {
    ...
    it('applies the device-local colour, labels and archived flag by key', () => {
        const keep: KeepNoteState = {
            colors: { 'list:1': 'mint' },
            labels: { 'list:1': ['Home', 'Errands'] },
            archived: { 'channel:9': true },
        };
        const out = cards([source('list', 1, 'a'), source('channel', 9, 'c')], {}, [], keep);
