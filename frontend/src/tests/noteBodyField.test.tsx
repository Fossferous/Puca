...
describe('NoteBodyField undo and redo', () => {
    it('undo puts the text back after a paste, and the save that follows writes THAT text', async () => {
        ...
        act(() => { vi.advanceTimersByTime(BODY_SAVE_DELAY_MS); });
        await flushPromises();
        // (the second argument is the base revision — none in this fixture)
        expect(onSave).toHaveBeenLastCalledWith('everything replaced by a paste', undefined);

        act(() => { histBtn('Undo')!.click(); });
        expect(area().value).toBe('my shopping list');
        act(() => { vi.advanceTimersByTime(BODY_SAVE_DELAY_MS); });
        await flushPromises();
        expect(onSave).toHaveBeenLastCalledWith('my shopping list', undefined);
