...
describe('NoteEditor: the base a rename names', () => {
    // `content.features` is read at the top of NoteEditor (the note's own
    // reminder, migration 068), so the fake has to carry it or the editor
    // cannot mount at all — nothing here is about note reminders.
    const stub = (): NoteActions => ({
        renameNote: vi.fn(async () => true),
        content: { features: { body: true, attachments: true, noteReminders: false } },
