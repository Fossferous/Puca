 ...
 *    (api/listContent.ts trashOrDeleteList), and forgets the note's colour and labels
...
beforeEach(() => { vi.clearAllMocks(); resetTrashProbe(); });
...
describe('actions.setPinnedMany', () => {
    it('pins several notes in ONE save, over the full order, keeping hidden notes’ slots', async () => {
        ...
        // A save waits until the server has said whether it has a trash (its
        // notes' slots must be kept): let the features answer land first.
...
describe('Undo of a delete that is still queued offline', () => {
    it('puts the note back on screen and queues the restore right behind the move', async () => {
        const qc = new QueryClient();
        qc.setQueryData<TaskList[]>(notesKeys.lists, [list(1), list(2), list(3)]);
        ...
        await act(async () => { ok = await actions().restoreNote({ kind: 'list', id: 2 }); });
        expect(ok).toBe(true);
        // The trash was never read here, so only the delete's own record could bring it back.
        expect(qc.getQueryData<TaskList[]>(notesKeys.lists)!.map(l => l.id).sort()).toEqual([1, 2, 3]);
        expect(vi.mocked(sendNoteOp).mock.calls.map(c => (c[0] as NoteOp).k)).toEqual(['deleteList', 'restoreList']);
        expect(forgetNoteKeys).not.toHaveBeenCalled();
    });
});
