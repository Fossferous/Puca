...
export function SelectionBar({ cards, actions, labels, bulk, onClear, onSelectAll, allSelected }: SelectionBarProps) {
    ...
    const deletable = cards.filter(c => c.ref.kind === 'list');
    const shared = cards.length - deletable.length;
    ...
    const duplicate = async () => {
        ...
        for (const c of cards) {
            if (await actions.createNote(`${c.title} (copy)`, openItemsOf(c))) made++;
        }
        ...
        <>
            <div className="notes-selectbar" role="toolbar" aria-label={`${cards.length} notes selected`}>
                ...
                <button
                    ...
                    title={deletable.length === 0 ? 'Shared notes can only be deleted from their server' : shared > 0 ? `Delete ${deletable.length} (shared notes are skipped)` : 'Delete'}
