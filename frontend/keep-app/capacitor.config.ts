...
/**
 * Púca Keep as its own Android app.
 *
 ...
 *     storage), so it has its own sign-in — Keep's login form takes the same
 *     Púca account;
 *   - none of Púca's native plugins (notifications, keep-alive, geofences,
 *     FCM, the OTA updater): Keep is a notes surface, and the web code
 *     degrades to "not available" where it asks for them.
 *
 * The web bundle is the Keep page built in NATIVE mode
 * (`KEEP_TARGET=native vite build --config vite.keep.config.ts`, base '/'
 * instead of '/keep/'), written to ../dist-keep-app. scripts/build-keep-app.mjs
 * runs the whole chain, CSP meta included.
 */
/**
 * KEEP_ALLOW_HTTP_API=1 is for a TEST build against a plain-http backend
 ...
 */
const ALLOW_HTTP_API = process.env.KEEP_ALLOW_HTTP_API === '1';

const config: CapacitorConfig = {
    appId: 'com.sovereign.keep',
    appName: 'Púca Keep',
    webDir: '../dist-keep-app',
    server: {
