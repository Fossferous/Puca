...
// notes via Keep, so the APK under test has "current notes" to show.
...
await p.goto('/keep/');
await p.waitForSelector('.keep-app', { timeout: 20000 });
const add = async (title, items) => {
    await p.click('.keep-quickadd-collapsed');
    await p.fill('.keep-quickadd-title', title);
    for (let i = 0; i < items.length; i++) {
        const inp = p.locator('.keep-quickadd-item input').nth(i);
        await inp.fill(items[i]);
        if (i < items.length - 1) await inp.press('Enter');
    }
    await p.getByRole('button', { name: 'Done' }).click();
    await p.waitForSelector(`.keep-card:has-text("${title}")`, { timeout: 15000 });
    console.log('note', title);
};
await add('Groceries', ['Milk', 'Bread', 'Eggs']);
await add('Weekend', ['Call Sam', 'Fix the bike']);
await p.locator('.keep-card', { hasText: 'Groceries' }).hover();
await p.locator('.keep-card', { hasText: 'Groceries' }).locator('.keep-card-pin').click();
